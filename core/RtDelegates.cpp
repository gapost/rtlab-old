...
/**************************************************************************************************/
DisplayDelegate::DisplayDelegate(QObject* parent, RtDataChannel* ch, QWidget* w) : QObject(parent),
channel_(ch), widget_(w)
{
	channel_->add_widget(this,this);
}
DisplayDelegate::~DisplayDelegate()
{
	if (channel_) channel_->remove_widget(this);
}
...
void DisplayDelegate::customEvent(QEvent* e)
{
...
PropertyDelegate::PropertyDelegate(QObject* parent, RtObject* obj, QMetaProperty p, QWidget* w, bool readOnly) : QObject(parent),
obj_(obj),property_(p),widget_(w), readOnly_(readOnly)
{
	...
	// if obj or widget dies, I die too
	connect(obj_,SIGNAL(destroyed()),this,SLOT(deleteLater()));
	connect(widget_.widget(),SIGNAL(destroyed()),this,SLOT(deleteLater()));
